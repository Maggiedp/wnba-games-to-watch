...
descriptor, and the two nearest stylistic neighbors. No I/O — the endpoint and
tests both call compute_style_view().
"""
...
# Radar order (clockwise from top). All six are "higher raw value -> more of the
# trait" so higher percentile == further out.
...
# (high-extreme phrase, low-extreme phrase) per axis.
_PHRASES: dict[str, tuple[str, str]] = {
    "pace": ("Up-tempo", "Deliberate"),
    "three_pa_rate": ("Perimeter-heavy", "Paint-oriented"),
    "ft_rate": ("Gets to the line", "Jump-shooting"),
    "oreb_pct": ("Crashes the glass", "Gets back on D"),
    "assist_rate": ("Ball-movement", "Iso-heavy"),
    "def_pressure": ("Ball-hawking D", "Contain D"),
}
...
def _neighbors(
    ...
    """The 2 nearest teams by Euclidean distance in the normalized 6-D space."""
    here = vecs[name]
    dists = []
    for other, vec in vecs.items():
        if other == name:
            continue
        d = math.dist(here, vec)
        dists.append((d, other))
    dists.sort(key=lambda t: (t[0], t[1]))
    return [{"abbr": abbrevs.get(o, ""), "team": o} for _d, o in dists[:2]]
...
def compute_style_view(
    ...
    team_id/team/abbr/games_played + the 6 raw metric keys in AXES. Returns one
